        ...
        const fetchProducts = async () => {
            try {
                const res = await fetch(
                    `${baseUrl}/product/getproducts?referenceWebsite=${referenceWebsite}&category=${encodeURIComponent("Cooking Appliances")}`
                );
    ...
    return (
        <div className="bg-gray-50 py-6 px-4">
            <div className="flex justify-between items-center mb-3  mx-20">
                <div>
                    <h2 className="text-2xl font-bold">Your Kitchen’s Coolest Upgrade</h2>
                    {/* <p className="text-sm text-gray-600">Starting at Rs. 9999*</p> */}
                </div>
                <Link to={`/category/${slugify("Cooking Appliances")}`} className="text-sm text-indigo-600 font-semibold hover:underline">View All →</Link>
            </div>
